...
class CustomStockEntry(StockEntry):
	# 🔵 Use purpose (more stable than stock_entry_type)
	def is_material_transfer_type(self):
		return self.purpose == "Material Transfer"
	...
	def set_rate_for_outgoing_items(self, reset_outgoing_rate=True, raise_error_if_no_rate=True):
		...
		for d in self.get("items"):
			if d.s_warehouse:
				# 🔵 Priority 1: custom_updated_rate (only if > 0)
				if flt(d.get("custom_updated_rate")) > 0:
					d.basic_rate = flt(d.custom_updated_rate)

				# 🔵 Priority 2: user manually set rate
				elif d.get("set_basic_rate_manually") and flt(d.basic_rate) > 0:
					pass  # keep user's basic_rate

				# 🔵 Priority 3: system valuation
